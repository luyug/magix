# import os
# # print host name
# # print(os.uname().nodename, flush=True)
# # os.environ.update({
...
#     # "--xla_gpu_threshold_for_windowed_einsum_mib=2048 "
...
#     # "--xla_gpu_collective_permute_decomposer_threshold=512 "
...
# # os.environ["NCCL_NET_GDR_LEVEL"] = "PIX"
...
# # os.environ["FI_CXI_REQ_BUF_SIZE"] = "268435456"
...
os.environ["XLA_FLAGS"] = (
    ...
    # "--xla_gpu_shard_autotuning=false "
    "--xla_gpu_enable_latency_hiding_scheduler=true "
    # "--xla_gpu_pgle_profile_file_or_directory_path=./temp.pbtxt"
    ...
    # "--xla_gpu_enable_while_loop_double_buffering=true "
    # "--xla_gpu_multi_streamed_windowed_einsum=true "
    # "--xla_gpu_threshold_for_windowed_einsum_mib=1024 "
    # "--xla_gpu_enable_all_gather_combine_by_dim=false "
    # "--xla_gpu_enable_reduce_scatter_combine_by_dim=false "
    # "--xla_gpu_enable_custom_fusions=true "
    # "--xla_allow_excess_precision=true "
    ...
    # "--xla_gpu_collective_permute_decomposer_threshold=512 "
    # "--xla_gpu_lhs_enable_gpu_async_tracker=true "
    # "--xla_gpu_enable_approx_costly_collectives=true "
    # "--xla_gpu_use_memcpy_local_p2p=true "
...
# os.environ["TF_CPP_VMODULE"] = "profile_guided_latency_estimator=10"
# os.environ["TF_CPP_MIN_LOG_LEVEL"] = "0"
# os.environ["TF_CPP_MAX_LOG_LEVEL"] = "100"

# os.environ['FI_CXI_DISABLE_HOST_REGISTER'] = '1'
# os.environ['FI_MR_CACHE_MONITOR'] = 'userfaultfd'
# os.environ['FI_CXI_DEFAULT_CQ_SIZE'] = '524288'
# os.environ['FI_CXI_RX_MATCH_MODE'] = 'hybrid'
# os.environ['FI_CXI_RDZV_PROTO'] = 'alt_read'
# os.environ['FI_CXI_REQ_BUF_SIZE'] = '268435456'
# os.environ['FI_MR_CACHE_MAX_SIZE'] = '2147483648'
# os.environ['FI_CXI_RDZV_THRESH'] = '262144'
# os.environ['FI_CXI_PROGRESS_MODE'] = 'auto'
# os.environ['FI_CXI_RX_SIZE'] = '32768'
# os.environ['FI_CXI_TX_SIZE'] = '32768'
# os.environ['FI_CXI_EAGER_PROTO'] = 'data'
# os.environ['FI_CXI_FORK_SAFE'] = '1'
# os.environ['FI_CXI_LAZY_MEM_REG'] = '1'
# os.environ['FI_CXI_DEDICATED_TX'] = '1'
# os.environ['FI_CXI_MULTI_RECV'] = '1'
# os.environ['FI_CXI_COPY_MODE'] = 'async'
# os.environ['FI_CXI_MULTI_SEND'] = '1'
# os.environ['FI_CXI_RMA_THRESHOLD'] = '262144'
...
from magix.spmd_utils import create_local_mesh, sub_mesh_map
...
OPT_SHARDING_RULE = {
    'embed_tokens/embedding': PS(('data', 'pipe'), 'model'),
    'lm_head/kernel': PS(('data', 'pipe'), 'model'),
...
COMPUTE_SHARDING_RULE = {
    ...
    'layernorm/weight': PS('pipe', 'model'),
}
...
def main():
    ...
    local_mesh = create_local_mesh(
        n_replicas, 
        (1, mesh.shape['pipe'], mesh.shape['model']),
        ('data', 'pipe', 'model')
    )
    ...
    @partial(
        ...
        out_shardings=(
            None, 
            magix.spmd_utils.get_sharding_tree(params, OPT_SHARDING_RULE, local_mesh),
        ),
        donate_argnums=(0, 1),
    )
    ...
    compute_grad = sub_mesh_map(
        compute_grad,
        mesh,
        local_mesh,
        global_specs=(
            magix.spmd_utils.get_sharding_tree(params, COMPUTE_SHARDING_RULE),
            PS('data'), PS('data')
        ),
        local_specs=(
            magix.spmd_utils.get_sharding_tree(params, OPT_SHARDING_RULE),
            PS('data'), PS('data')
        ),
        out_specs=(
            PS('data'),
    ...
    def decide_scatter_dim(spec):
        if spec is None:
            return None
        for i, s in enumerate(spec):
            if s == 'data':
                return i
            if isinstance(s, tuple) and s[0] == 'data':
                return i
        return None
    
    grad_scatter_axes = jax.tree.map(
        decide_scatter_dim,
        magix.spmd_utils.get_sharding_tree(params, OPT_SHARDING_RULE),
    ...
    def optimizer_step(params, opt_state, replica_losses, replica_grads):
        loss = shard_map(
            lambda x: lax.pmean(x, 'data'),
            mesh=mesh, in_specs=PS('data'), out_specs=PS())(replica_losses)
        @partial(
            shard_map, mesh=mesh,
            in_specs=(magix.spmd_utils.get_sharding_tree(replica_grads, OPT_SHARDING_RULE),),
            out_specs=magix.spmd_utils.get_sharding_tree(replica_grads, OPT_SHARDING_RULE),
        )                                                
        def reduce_grads(gg):
            def _reduce_one(x, scatter_dim):
                if scatter_dim is None:
                    return lax.pmean(x, 'data')
                else:
                    return lax.psum_scatter(x, 'data', scatter_dimension=scatter_dim, tiled=True) / lax.psum(1, 'data')
            return jax.tree_map(_reduce_one, gg, grad_scatter_axes)
        
        
        grads = reduce_grads(replica_grads)
        metrics = {"loss": loss}
    ...
    def train_step(params, opt_state, batch):
        ...
        attention_mask = np.logical_and(
            batch['attention_mask'][:,:-1],
            batch['attention_mask'][:,1:]
        ).astype('bool')
        
        input_ids, attention_mask = jax.experimental.multihost_utils.host_local_array_to_global_array(
            (input_ids, attention_mask), mesh, PS(),
        )
        ...
        with jax.named_scope('OptimizerStep'):
            new_params, new_opt_state, metrics = optimizer_step(params, opt_state, loss, grads)
    ...
    for epoch in epochs:
        ...
        for step in trange(steps_per_epoch, disable=jax.process_index() != 0):
            ...
            if step in train_args.profiling_steps:
                ...
                params, opt_state, metrics = train_step(params, opt_state, _batch)
