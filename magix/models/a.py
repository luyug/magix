...
def decode_attn_unbatched(
  ...
  print('----->>>>| q.shape: ', q.shape, q, flush=True)
  print('----->>>>| k.shape: ', k.shape, k, flush=True)
  print('----->>>>| v.shape: ', v.shape, v, flush=True)
  print('----->>>>| start_idx.shape: ', start_idx.shape, start_idx, flush=True)
  print('----->>>>| kv_seq_len.shape: ', kv_seq_len.shape, kv_seq_len, flush=True)
  print('--------------------------------', flush=True)
  ...
  print('--->>>>>> o.shape: ', o.shape, o, flush=True)
...
def gqa(
  ...
  print('----->>>> q_reshaped.shape: ', q_reshaped.shape, q_reshaped, flush=True)
  print('----->>>> k_transposed.shape: ', k_transposed.shape, k_transposed, flush=True)
  print('----->>>> v_transposed.shape: ', v_transposed.shape, v_transposed, flush=True)
  print('--------------------------------', flush=True)
  ...
  o = jax.vmap(with_kv_heads)(
      q_reshaped, k_transposed, v_transposed, start_idx, kv_seq_len
  )
  assert q_reshaped.shape[0] == batch_size
  assert k_transposed.shape[0] == batch_size
  assert v_transposed.shape[0] == batch_size
  assert start_idx.shape[0] == batch_size
  assert kv_seq_len.shape[0] == batch_size
